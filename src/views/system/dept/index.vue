...
export default {
  ...
  cruds() {
    return CRUD({ title: '部门1', url: 'api/dept', crudMethod: { ...crudDept }})
  },
  ...
  methods: {
    ...
    getDepts() {
      ...
      crudDept.getDepts({ enabled: true }).then(res => {
        this.depts = res.content.map(function(obj) {
          if (obj.hasChildren) {
			obj.children = that.getChild(obj)
          }
          return obj
        })
      })
	...
	getChild(item){
		let that = this
	  crudDept.getDepts({ enabled: true, pid: item.id }).then(res => {
		item.children = res.content.map(function(obj) {
		  if (obj.hasChildren) {
			obj.children = that.getChild(obj)
		  }
		  return obj

...
function CRUD(options) {
	...
	const data = {
		...
		status: {
			...
			// 标题
			get title() {
				return this.add > CRUD.STATUS.NORMAL ? `新增${crud.title}` : this.edit > CRUD.STATUS.NORMAL ?
					`编辑${crud.title}` : crud.title
			}
		},
		msg: {
			submit: '提交成功',
	...
	const methods = {
		...
		bind() {
			let data = {
				phone: crud.form.phone
			}
			crud.crudMethod.getOne(data).then((res) => {
				res.bind = 1
		...
		toAdd(data) {
			...
			if (data) {
				...
				crud.resetForm(JSON.parse(JSON.stringify(dataV)))
			} else {
				crud.resetForm()
			}
			if (!(callVmHook(crud, CRUD.HOOK.beforeToAdd, crud.form) && callVmHook(crud, CRUD.HOOK.beforeToCU, crud
					.form))) {
				return
			}
		...
		toEdit(data) {
			...
			if (data.configId && data.num==0){
				data.num = ''
			}
			data.peizhi = 1
			crud.resetForm(JSON.parse(JSON.stringify(data)))
			if (!(callVmHook(crud, CRUD.HOOK.beforeToEdit, crud.form) && callVmHook(crud, CRUD.HOOK.beforeToCU, crud
					.form))) {
				return
			}
		...
		getQueryParams: function() {
			...
			Object.keys(crud.params).length !== 0 && Object.keys(crud.params).forEach(item => {
				if (crud.params[item] === null || crud.params[item] === '') crud.params[item] =
					undefined
			})
			return {
		 ...
		 */
		resetForm(data) {
			const form = data || (typeof crud.defaultForm === 'object' ? JSON.parse(JSON.stringify(crud
				.defaultForm)) : crud.defaultForm.apply(crud.findVM('form')))
